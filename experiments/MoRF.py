...
importance, we are removing features starting from the most important
XGBoost:
...
for f in range(9):
        ...
        feat_impor = pd.DataFrame(index = X.columns.to_list(), columns = split_i) 
        ...
        for i in range(n,m):
            ...
            feat_impor.loc[:,i] = model.feature_importances_

            results.loc[i, "mean_inf"] = np.mean(feat_imp[:n_inf])
            results.loc[i, "mean_Ninf"] = np.mean(feat_imp[n_inf:])
            results.loc[i, "removed_feature"] = f
            
        feat_impor['mean_f']=feat_impor.mean(axis=1)
        ind_imp = feat_impor.index[np.argmax(feat_impor.mean_f)]
        X = X.drop(ind_imp, axis=1)
        n=n+iterations
        m =m+iterations
        
        res_dir = './results/'
        
        results.to_csv(res_dir+ 'results_XGBoost_inf+MoRF' +'.csv')
...
X, y, c = make_class_dataset(_n_samples=1000, n_feat=n_feat,n_inf=n_inf,n_red=n_red, 
                       n_rep=n_rep, n_clas =6,class_sepr=class_sep,seed=seed)
...
for f in range(9):
        ...
        for i in range(n,m):
            ...
            X_train, X_test, X_val, y_train_enc, \
            y_val_enc, y_test_enc =splitting_data( data_x = X, data_y = pd.Series(y), 
                                                  val_split = 0.1,test_split = 0.2,random_seed =42)
            ...
            #results.loc[i, "c_ind_Ninf"] =concordance_index(c[n_inf:],attn[n_inf:])
            results.loc[i, "c_ratio"] =np.sum(attn[:n_inf]) / np.sum(attn[n_inf:])
            

            attn =model_go.layers[1].weights[1].numpy()
            attn = np.abs(attn).flatten().tolist()
            results.loc[i, "mean_inf"] = np.mean(attn[:n_inf])
            results.loc[i, "mean_Ninf"] = np.mean(attn[n_inf:])
            results.loc[i, "n_feat"] = f 

            feat_impor.loc[:,i] = attn
            results.loc[i, "removed_feature"] = f
        ...
        results.to_csv(res_dir+ 'results_pnet_inf+MoRF' +'.csv')
