...
for i in range(n):
    ...
    att_weight = pd.DataFrame()
    att_weight["genes"] = go2.index
    att_weight["attw"] = model_go.layers[2].weights[0].numpy()
    att_weight.rename(columns={'attw': "split_" +str(i),
                    'genes': "genes"}, inplace=True)
    attn_weights = attn_weights.merge(att_weight, how='left', on='genes')
    
    
    
print("--- %s seconds ---" % (time.time() - start_time))
res_dir = "./results/"
attn_weights.to_csv(res_dir + "metabric_attnw_bad.csv") 
results.to_csv(res_dir + "metabric_attw_extr_results.csv")  
